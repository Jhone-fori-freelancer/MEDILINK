...
function CustomHeaderComponent() {
  ...
  return (
    <thead>
      <tr className="flex gap-2 w-full justify-between">
        {days.map((day, index) => (
...
const CALENDAR_CLASSNAMES = {
  ...
  day_today: "text-white !opacity-100 !bg-[#5C5C5C]",
  day_outside:
...
function Calendar({
  ...
  ...props
}: CalendarProps & { appointmentsStatus?: { [key: string]: string } } & { waitingListDays?: Date[] } & { errorDays?: Date[] } & { isDoctor?: boolean } & { availableDays?: Date[] }) {
  const currentMonth = new Date()
  const nextMonth = new Date(currentMonth)
  nextMonth.setMonth(currentMonth.getMonth() + 1)
  const secondNextMonth = new Date(currentMonth)
  secondNextMonth.setMonth(currentMonth.getMonth() + 2)
  console.log('available days', props.availableDays);
  ...
  return (
    <>
      <DayPicker
        ...
        modifiers={{
          waiting: (date) =>
            !isDisabled(date) && !!props.waitingListDays?.some(d => d.toDateString() === date.toDateString()),
          error: (date) =>
            !isDisabled(date) && !!props.errorDays?.some(d => d.toDateString() === date.toDateString()),
          available: (date) =>
            !!props.availableDays?.some(d => d.toDateString() === date.toDateString())
        }}
        modifiersClassNames={{
          waiting: `relative after:text-[10px] ${props.isDoctor ? "after:content-['Pocos_turnos_disponibles']" : "after:content-['Lista_de_espera_abierta']"} after:z-10  after:absolute after:text-xs after:font-semibold after:text-start after:top-[30%] after:bg-[#FFCC99] after:text-black after:w-[98%] after:left-[1.5px] after:h-[48px] after:rounded-md after:text-start after:p-1 after:text-wrap`,

          error: `relative after:text-[10px] ${props.isDoctor ? "after:content-['Agenda_completa']" : "after:content-['Sin_turnos_disponibles']"} after:z-10 after:absolute after:text-xs after:font-semibold after:text-start after:top-[30%] after:bg-[#FFB4B2] after:text-black after:w-[98%] after:left-[1.5px] after:h-[48px] after:rounded-md after:text-start after:p-1 after:text-wrap`,
        }}
